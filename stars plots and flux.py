...
def flux(x, y):
    ...
    aver_sum = 0
    stars_sum = 0
    for i in range(x - big_r, x + big_r + 1):
        for j in range(y - big_r, y + big_r + 1):
            aver_sum += scidata[j, i]

    for i in range(x - stars_r, x + stars_r + 1):
        for j in range(y - stars_r, y + stars_r + 1):
            stars_sum += scidata[j, i]

    ring_value = aver_sum - stars_sum   # количество фотонов в кольце
    ring_space = (2*big_r + 1) ** 2 - (2*stars_r + 1)**2  # площадь кольца
    density = ring_value / ring_space
    noise = density * (2*stars_r + 1)**2
    
    flux = stars_sum - int(noise)
                           
    flux_label = tk.Label(win,
                          text=f'{flux}',
                          bg='Plum',
                          fg='DarkSlateBlue')
    flux_label.grid(row=4, column=5, stick='wnes')
...
coordinates_label = tk.Label(win,
                             text='Введите координаты объектов:',
                             bg='LightSeaGreen',
                             fg='MidnightBlue'
                             )
...
star_r_entry = tk.Entry(win,
                        bg='Plum')
star_r_entry.insert(tk.INSERT, '6')
...
big_r_btn.grid(row=3, column=6, stick='n')
